...
const jsonDbPath = './data/films.json';
...
router.get('/:id', function (req, res) {
  ...
  if (req.params.id === undefined) {
    return res.sendStatus(400);
  } else if (req.params.id < 1 || req.params.id > films.length) {
    res.sendStatus(404);
  }
...
router.delete('/:id', function (req, res) {
  ...
  if (isNaN(filmIndex) || filmIndex === undefined) {
    return res.sendStatus(400);
  } else if (filmIndex < 1 || filmIndex > films.length) {
    res.sendStatus(404);
  }
...
router.patch('/:id', function (req, res) {
  ...
  if (title?.length === 0 || duration?.length === 0 || budget?.length === 0 || link?.length === 0) {
    return res.sendStatus(400);
  }
...
router.put('/:id', function (req, res) {
  ...
  if (!id && !title && !duration && !budget && !link) {
    return res.sendStatus(400);
  }
  if (title?.length === 0 || duration?.length === 0 || budget?.length === 0 || link?.length === 0) {
    return res.sendStatus(400);
  ...
  const filmIndex = films.findIndex((film) => film.id === parseInt(id));
